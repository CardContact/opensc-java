/***********************************************************
 * $Id: PKCS11Slot.java 62 2007-02-15 08:14:53Z wolfgang.glas $
 * 
...
public class PKCS11Slot extends DestroyableHolder
{
	...
	/**
	 * This constructor constructs an instance of an individual slot.
	...
	private native void initTokenNative(long _pvh, long _handle, byte[] soPIN, byte[] label) throws PKCS11Exception;

	/**
	 * Initialize token and set soPIN and label.
	 * If the token does not exists, then it is created and the SO PIN is set to the
	 * value provided. If the token already exists, then the SO PIN provided is used
	 * to verify the current PIN.
	 * 
	 * @param soPIN the SO PIN
	 * @param label the new token label
	 * @throws PKCS11Exception Upon errors during token initialization
	 */
	public void initToken(char[] soPIN, String label) throws PKCS11Exception {
		initTokenNative(this.pvh, this.handle, Util.translatePin(soPIN), Util.translatePin(label.toCharArray()));
	}
