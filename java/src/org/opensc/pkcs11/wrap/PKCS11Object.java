/***********************************************************
 * $Id: PKCS11Object.java 82 2007-03-21 21:24:35Z andreas.schwier $
 * 
...
public class PKCS11Object extends DestroyableChild implements PKCS11SessionChild
{
	...
	private static native void deleteObjectNative(long pvh, long slot_handle, long hsession, long handle) throws PKCS11Exception;
	 ...
	 * @throws PKCS11Exception Upon errors of the underlying PKCS#11 module.
	 */
	protected static long[] enumRawObjects(PKCS11Session session, PKCS11Attribute[] attrs) throws PKCS11Exception
	{
		return enumObjectsNative(session.getPvh(),session.getSlotHandle(),session.getHandle(),
				attrs);
	}
	...
	protected PKCS11Object(PKCS11Session session, long handle) throws PKCS11Exception
	{
		...
		// TODO code should move to derived class, as not all objects have CKA_ID and CKA_LABEL
		try
		{
	...
	{
		deleteObjectNative(this.pvh,this.shandle,this.hsession,this.handle);
	}
	...
	/* (non-Javadoc)
	 * @see org.opensc.pkcs11.wrap.PKCS11SessionChild#getSession()
	 */
	public PKCS11Session getSession() {
		DestroyableParent parent = this.getParent();
		if (parent instanceof PKCS11Session) {
			return (PKCS11Session)parent;
		}
		return null;
	}
