...
JNIEXPORT void JNICALL JNIX_FUNC_NAME(Java_org_opensc_pkcs11_wrap_PKCS11Slot_initTokenNative)
  (JNIEnv *env, jobject jslot, jlong mh, jlong handle, jbyteArray soPIN, jbyteArray jlabel)
{
  int rv;
  jsize sz;
  ...
  if (soPIN)
  {
    allocaCArrayFromJByteArray(pin,pin_len,env,soPIN);
  }

  memset(label, ' ', sizeof(label));

  sz = (*env)->GetArrayLength(env,jlabel);

  if (sz > 32)
  {
    jnixThrowException(env,"java/lang/IllegalArgumentException",
                        "Length of label exceeds 32 bytes");
    return;
  }

  (*env)->GetByteArrayRegion(env,jlabel,0,sz,(jbyte*)label);

  rv = mod->method->C_InitToken(slot->id, pin, pin_len, label);

  if (rv  != CKR_OK)
    {
      jnixThrowExceptionI(env,"org/opensc/pkcs11/wrap/PKCS11Exception",rv,
                          "C_InitToken failed for module " PKCS11_MOD_NAME_FMT ".",
                          mod->name);
      return;
    }
}
